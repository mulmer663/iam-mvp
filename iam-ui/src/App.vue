...
// Auto-scroll when panes change
watch(
  () => millerStore.panes.length, 
        ...
        <div class="h-full flex gap-2 w-max">
           <TransitionGroup name="pane">
             <div 
               ...
               :key="pane.id"
               class="h-full bg-white border border-neutral-200 rounded-lg shadow-sm flex flex-col transition-all duration-300 ease-out pane-enter-active pane-leave-active"
               :class="[
                  millerStore.activePaneId === pane.id ? 'ring-2 ring-blue-500/20 border-blue-400 shadow-md z-10' : 'opacity-80 scale-[0.99] hover:opacity-100 hover:scale-100',
                  pane.type === 'UserDetail' ? 'w-[800px]' : '' // Specific width for UserDetail
               ]"
