...
public class UserQueryService {
    ...
    private ScimUserResponse toScimUser(IamUser user) {
        ...
        return ScimUserResponse.builder()
                ...
                .emails(List.of(new ScimUserResponse.Email(user.getUserName(), true))) // Map userName to email as per
                                                                                       // plan
                .active(user.isActive())
                .enterpriseExtension(enterpriseData)
                .meta(new ScimUserResponse.Meta(
                        "User",
                        user.getCreated() != null ? user.getCreated().format(DateTimeFormatter.ISO_DATE_TIME) : null,
                        user.getLastModified() != null ? user.getLastModified().format(DateTimeFormatter.ISO_DATE_TIME)
                                : null,
