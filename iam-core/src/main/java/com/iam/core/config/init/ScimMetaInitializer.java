...
public class ScimMetaInitializer implements CommandLineRunner {
        ...
        private void initAttributeMeta() {
                ...
                List<IamAttributeMeta> attributes = new java.util.ArrayList<>();
                ...
                // Addresses (Specific sub-attributes)
        ...
        private List<IamAttributeMeta> createSubAttrs(String parentName, String schema, String... subNames) {
                java.util.List<IamAttributeMeta> subs = new java.util.ArrayList<>();
                for (String subName : subNames) {
                        // ID should be parent.child to be unique in DB (Project Rule)
                        // SCIM name logic in ScimSchemaService strips the prefix.
                        String id = parentName + "." + subName;
                        subs.add(IamAttributeMeta.builder()
                                        .name(id)
                                        .parentName(parentName)
                                        .targetDomain(AttributeTargetDomain.USER) // Inherit?
                                        .category(AttributeCategory.CORE)
                                        .displayName(subName) // Simple display name
                                        .type(AttributeDataType.STRING) // Default to String for sub-attributes mostly
                                        ...
                                        .multiValued(false) // Sub-attributes are usually singular within the complex
                                                            // object
                                        .build());
                }
                return subs;
